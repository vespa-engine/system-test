...
class HttpClient
  ...
  def getcmd(file, host, port, route, num_connections)
    "java -server -verbose:gc -XX:NewRatio=1 -Xms8g -Xmx8g -cp #{CLIENT_LIB} #{CLIENT_CLASS} --vespaTls --file #{file} --host #{host} --port #{port} --route #{route} -v --numPersistentConnectionsPerEndpoint #{num_connections} --maxpending 20000"
  end
