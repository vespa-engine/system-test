...
class GeneratorTest < Test::Unit::TestCase
  ...
  def test_query
    generator = Generator.new
    query = 'sddocname:test foo:$words() bar:$words() i:$rseq()'
    query_command = generator.query_command(template: query, count: 2, parameters: {:a => 'b'})
    assert_equal(`#{query_command}`,
                 "/search/?query=sddocname%3Atest+foo%3Ared+bar%3Atwitter+i%3A0&a=b\n" +
                 "/search/?query=sddocname%3Atest+foo%3Aat+bar%3Adefensive+i%3A1&a=b\n")
  end
