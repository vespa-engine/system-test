...
class BasicContainer < PerformanceTest
  ...
  def benchmark_queries(template, yql)
    setup_and_deploy(@app)
    container = (vespa.qrserver['0'] or vespa.container.values.first)
    @queryfile = dirs.tmpdir + "/queries.txt"
    container.write_queries(template: template, yql: yql, count: 1000000, filename: @queryfile)
    profiler_start
  ...
  def test_container_search_performance
    set_description('Test basic search container with libyell and simple query parsing. Uses a Simple Searcher with Mock Hits')
    benchmark_queries('text:$words() AND text:$words() AND text:$words()', false)
  end

  def test_container_yql_performance
    set_description('Test basic search container with libyell and YQL query parsing. Uses a Simple Searcher with Mock Hits')
    benchmark_queries('select * from sources * where text contains "$words()" AND text contains "$words()" AND text contains "$words()";', true)
  end
