...
class CommonSiftGistBase < CommonAnnBaseTest
  ...
  def query_and_benchmark(algorithm, target_hits, explore_hits, filter_percent = 0, approximate_threshold = 0.05, filter_first_threshold = 0.0, filter_first_exploration = 0.01, slack = 0.0, clients = 1, threads_per_search = 0)
    ...
    label = "#{algorithm}-th#{target_hits}-eh#{explore_hits}-f#{filter_percent}-at#{approximate_threshold}-fft#{filter_first_threshold}-ffe#{filter_first_exploration}-n#{clients}-sl#{slack}-t#{threads_per_search}"
