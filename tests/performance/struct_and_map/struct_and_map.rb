...
class StructAndMapPerf < PerformanceTest
  ...
  def test_matched_elements_only
    ...
    run_fbench(container, 8, 20, [], {:append_str => "&timeout=10.0&hits=10&summary=filtered_map_attr" })

    profiler_start
    run_fbench(container, 32, 60, [parameter_filler('legend', MAP_ATTR)], {:append_str => "&summary=filtered_map_attr"})
    profiler_report(MAP_ATTR)

    @queryfile = selfdir + 'query_map_mix.txt'
    profiler_start
    run_fbench(container, 32, 60, [parameter_filler('legend', MAP_MIX)], {:append_str => "&summary=filtered_map_mix"})
