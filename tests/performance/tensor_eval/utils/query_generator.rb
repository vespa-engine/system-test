...
def self.gen_dot_product_wset_query(query_vector)
  "/search/?query=wset_entries:#{query_vector.size}&rankproperty.dotProduct.wset_query=#{gen_wset(query_vector)}"
end

def self.gen_dot_product_array_query(query_vector)
  "/search/?query=wset_entries:#{query_vector.size}&rankproperty.dotProduct.array_query=#{gen_dot_product_array(query_vector)}"
end
...
def self.gen_tensor_dense_unbound_query(query_vector)
  "/search/?query=wset_entries:#{query_vector.size}&q_dense_vector_unbound=#{gen_tensor_array(query_vector)}"
end
...
def self.gen_tensor_sparse_query_y(query_vector)
  gen_tensor_sparse_query(query_vector, "y")
end

def self.gen_tensor_sparse_query(query_vector, dimension)
  "/search/?query=wset_entries:#{query_vector.size}&q_sparse_vector_#{dimension}=#{gen_tensor_array(query_vector)}"
end
...
def self.write_query_files(folder)
  ...
  [5,10,25,50,100,250,500].each do |num_entries|
    ...
    write_query_file("#{folder}queries.tensor.dense_unbound.#{num_entries}.txt", num_queries, num_entries, :gen_tensor_dense_unbound_query, query_vectors)
