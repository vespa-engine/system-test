...
class MixedTensorPerfTestBase < PerformanceTest
  ...
  def feed_and_profile(data_gen_params, feed_type)
    command = "#{@data_gen} #{data_gen_params}"
    profiler_start
    run_stream_feeder(command, [parameter_filler(FEED_TYPE, feed_type)], {})
    profiler_report("#{feed_type}")
  end

  def get_feed_throughput_graph(feed_type, y_min, y_max)
    {
      :x => FEED_TYPE,
      :y => "feeder.throughput",
      :title => "Throughput during feeding of '#{feed_type}' to mixed tensor",
