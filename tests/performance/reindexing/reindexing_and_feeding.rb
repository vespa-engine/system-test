...
class ReindexingAndFeedingTest < PerformanceTest
  ...
  def feed_data(config)
    run_feeder(config[:file],
	       ...
	       config.merge({ :localfile => true, :numthreads => 8, :feed_node => @qrserver, :client => :vespa_http_client, :numconnections => 8}))
  end
  ...
  def graph_config
	...
	'feeder.throughput'     =>   { }
      ...
      { :legend => 'reindex' } => {
	'reindexing.throughput' =>   { :y_min =>  3600, :y_max =>  7000 },
      },
      { :legend => 'feed' } => {
	...
	'feeder.throughput'     =>   { :y_min => 24000, :y_max => 36000 },
      }
    }
  end
