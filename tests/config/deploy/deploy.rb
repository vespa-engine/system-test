...
include ApplicationV2Api
  ...
  def test_binary_content
    ...
    assert_put_application_file(session_id, "#{CONFIG_DEPLOY_APP}app_b/components/test.jar", file)

    output = read_application_file(session_id, file)
    file = File.open("#{CONFIG_DEPLOY_APP}/app_b/#{file}", "rb")
    data = file.read
    file.close
  ...
  def run_single_session(session_id=@session_id)
    ...
    session_id = deploy_and_activate_session("#{CONFIG_DEPLOY_APP}/app_a", session_id, 1337)
    session_id = deploy_and_activate_session("#{CONFIG_DEPLOY_APP}/app_b", session_id, 1338)
    deploy_and_activate_session("#{CONFIG_DEPLOY_APP}/app_c", session_id, 1339)
  end
  ...
  def run_invalid_app(session_id=@session_id)
    ...
    result = create_session("#{CONFIG_DEPLOY_APP}/app_invalid", session_id)
  ...
  def run_multiple_create_sessions(session_id=@session_id)
    ...
    result_a = create_session("#{CONFIG_DEPLOY_APP}/app_a", session_a)
    result_b = create_session("#{CONFIG_DEPLOY_APP}/app_b", session_b)
  ...
  def run_activate_without_prepare(session_id=@session_id)
    ...
    result = create_session("#{CONFIG_DEPLOY_APP}/app_b", session_id)
  ...
  def run_serverdb_reuse_session_id(session_id=@session_id)
    ...
    deploy_and_activate_session("#{CONFIG_DEPLOY_APP}/base", session_id, 19080)
    ...
    session_id = 2
    create_tenant_and_wait(@tenant_name, @node.hostname)
    next_session_id = deploy_and_activate_session("#{CONFIG_DEPLOY_APP}/app_a", session_id, 1337) 
  ...
  def run_configserver_restart(session_id=@session_id)
    ...
    expected_port = 19080
    session_id = deploy_and_activate_session("#{CONFIG_DEPLOY_APP}/base", session_id, expected_port) 
    ...
    result = create_session("#{CONFIG_DEPLOY_APP}/app_a", session_id)
    restart_config_server(@node, :keep_zookeeper_data => true, :keep_configserver_data => true)
  ...
  def run_content_edit(session_id=@session_id)
    ...
    create_result = create_session("#{CONFIG_DEPLOY_APP}/app_a", session_id)

    # Get deployed file
    output = read_application_file(session_id, services)
    assert_file_content_equal("#{CONFIG_DEPLOY_APP}/app_a/#{services}", output, "Files were not equal")

    # Put a new file that did not exist
    assert_put_application_file(session_id, "#{CONFIG_DEPLOY_APP}app_b/#{services}", services)

    output = read_application_file(session_id, services)
    assert_file_content_equal("#{CONFIG_DEPLOY_APP}/app_b/#{services}", output, "Files were not equal")
    ...
    assert_put_application_file(session_id, "#{CONFIG_DEPLOY_APP}/extra/test1.txt", "files/test1.txt")
  ...
  def run_content_status(session_id=@session_id)
    ...
    assert_put_application_file(session_id, "#{CONFIG_DEPLOY_APP}/extra/test1.txt", test_file)
  ...
  def run_zip_compression(session_id=@session_id)
    ...
    result = create_session("#{CONFIG_DEPLOY_APP}/app_a", session_id, "zip")
    result = prepare_session(result, session_id)
    result = activate_session(result, session_id)
    assert_logd_config(1337)
    next_session(session_id)
  ...
  def run_outdated_session_activation(session_id=@session_id)
    ...
    create_result_a = create_session("#{CONFIG_DEPLOY_APP}/app_a", session_a)
    ...
    deploy_and_activate_session_v2("#{CONFIG_DEPLOY_APP}/app_c", session_c, 1339)
    ...
    deploy_and_activate_session_v2("#{CONFIG_DEPLOY_APP}/app_b", session_d, 1338)
  ...
  def run_application_list(session_id=@session_id)
    ...
    session_id = deploy_and_activate_session_v2("#{CONFIG_DEPLOY_APP}/app_a", session_id, 1337)
    apps = list_applications_v2(@hostname, @tenant_name)
    assert_equal(1, apps.length)
  ...
  def run_delete_application(session_id=@session_id)
    ...
    session_id = deploy_and_activate_session_v2("#{CONFIG_DEPLOY_APP}/app_b", session_id, 1338)
    apps = list_applications_v2(@hostname, @tenant_name)
    assert_equal(1, apps.length)
    assert_equal(application_url(@hostname, @tenant_name), apps[0])
  ...
  def run_application_content(session_id=@session_id)
    ...
    assert_file_content_equal("#{CONFIG_DEPLOY_APP}/app_a/#{services}", response.body, "Files were not equal")
    output = JSON.parse(put_file("#{application_url(@node.hostname)}/content/#{services}", "#{CONFIG_DEPLOY_APP}/app_b/#{services}"))
  ...
  def run_old_sessions_are_deleted(session_id=@session_id)
    ...
    session_id = deploy_and_activate_session_v2("#{CONFIG_DEPLOY_APP}/app_c", session_id, 1339)
  ...
  def run_create_two_sessions_activate_second_then_first(session_id=@session_id)
    ...
    create_result = create_session("#{CONFIG_DEPLOY_APP}/app_a", first_session)
    ...
    deploy_and_activate_session_v2("#{CONFIG_DEPLOY_APP}/app_c", second_session, 1339)
  ...
  def run_prepare_and_activate_in_one_call(session_id=@session_id)
    ...
    session_id = prepare_and_activate_one_call("#{CONFIG_DEPLOY_APP}/app_a", session_id, 1337)
    next_session(session_id)
  end
  ...
  def run_create_prepare_and_activate(session_id)
    ...
    result = create_prepare_and_activate("#{CONFIG_DEPLOY_APP}/app_a", @hostname, @tenant_name)
    assert_logd_config(1337)
    next_session(session_id)
  end
