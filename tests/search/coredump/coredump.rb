...
class CoreDump < IndexedOnlySearchTest
  ...
  def test_coredump_compression
    ...
    core_dump = VespaCoredump.new(File.dirname(fullcorefile_uncompressed), File.basename(fullcorefile_uncompressed), "vespa-proton-bin")
    puts "#{fullcorefile_uncompressed}"
    puts "#{core_dump}"
    puts "#{core_dump.stacktrace}"
    assert_match(/Core was generated by/, core_dump.stacktrace, "Unexpected stacktrace")
