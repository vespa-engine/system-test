...
class Phrases < IndexedStreamingSearchTest
  ...
  def test_phrases
    ...
    start
    feed_and_wait_for_docs('music', 256, :file => selfdir+'docs-phrases.json')

    docs = []
    File.open(selfdir+'docs-phrases.json') do |file|
      file.each do |line|
        if (line.match(/"title":"([^"]*)"/))
          docs.push($1)
        end
  ...
  def test_phrase_rank
    ...
    start
    feed_and_wait_for_docs('phraserank', 4, :file => selfdir + 'docs-phraserank.json')
