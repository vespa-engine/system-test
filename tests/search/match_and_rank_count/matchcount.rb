...
class MatchCount < IndexedSearchTest
  ...
  def verify_counting(type)
    ...
    set_counts(expected.hit[0],7,0,0,0)
    set_counts(expected.hit[1],7,0,0,0)
    verify_query(query, 2, expected)
    verify_query(query, 2, expected)

    set_counts(expected.hit[0],8,0,0,0)
    set_counts(expected.hit[1],8,0,0,0)
    verify_query(query, 2, expected, "%2b%2b")
    set_counts(expected.hit[0],9,0,1,0)
    set_counts(expected.hit[1],9,0,0,0)
    verify_query(query, 2, expected, "%2b%2b", "%2b%2b")
    set_counts(expected.hit[0],10,0,2,0)
    set_counts(expected.hit[1],10,0,0,0)
    verify_query(query, 2, expected, "%2b%2b", "%2b%2b")
    set_counts(expected.hit[0],11,0,2,0)
    set_counts(expected.hit[1],11,0,0,0)
    verify_query(query, 2, expected, "%2b%2b")
    set_counts(expected.hit[0],11,0,2,1)
    set_counts(expected.hit[1],11,0,0,1)
    verify_query(query, 2, expected, nil, nil, "%2b%2b")
    set_counts(expected.hit[0],0,0,2,1)
    set_counts(expected.hit[1],0,0,0,1)
  ...
  def test_on_xxx_in_rank_profile
    ...
    r = search(query+"&ranking=rank1")
    set_counts(expected.hit[0],8,1,1,1)
    set_counts(expected.hit[1],8,1,0,1)
    assert_equal(expected, r.hit)

    r = search(query+"&ranking=rank2")
    r = search(query+"&ranking=rank1")
    set_counts(expected.hit[0],9,2,2,2)
    set_counts(expected.hit[1],9,2,0,2)
    assert_equal(expected, r.hit)

    search(query+"&ranking=rank2&hits=1")
    r = search(query+"&ranking=rank1")
    set_counts(expected.hit[0],10,3,3,3)
    set_counts(expected.hit[1],10,3,0,2)
    assert_equal(expected, r.hit)
  end
