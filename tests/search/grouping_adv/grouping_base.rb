...
module GroupingBase
  ...
  def querytest_filter
    check_query('all(group(sf) filter(range(2.9, 8.9, sf)) each(output(count())))', 'range-1', 40)
    check_query('all(group(sf) filter(range(2.9, 8.9, sf, true, false)) each(output(count())))', 'range-1', 40) # default values for inclusion is true, false
    check_query('all(group(sf) filter(range(2.9, 8.9, sf, false, true)) each(output(count())))', 'range-2', 40)
    ...
    check_query('all(group(a) filter(regex("^a*$", s) and regex("^b1$", b)) each(output(count())))', 'predicate-3')
