...
module GroupingBase
  ...
  def querytest_common
    ...
    # Test quantiles
    check_query('all(group(boool) each(output(quantile(0.1, f))))', 'quantile-1', 40)
    check_query('all(group(boool) each(output(quantile(0.5, f))))', 'quantile-2', 40)
    check_query('all(group(boool) each(output(quantile(0.9, f))))', 'quantile-3', 40)
