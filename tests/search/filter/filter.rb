...
class Filter < IndexedSearchTest
  ...
  def test_filter
    ...
    puts "Sanity check"
    assert_hitcount("/?query=sddocname:f", 7)

    result = search("/?query=title:something+artist:foobar")
    ...
    result = search("/?query=title:something&filter=%2Bartist:foobar")
    assert_equal([ "5 foobar foobar",
                   "4 foobar",
                   "2 x x x x foobar",
                   "7 foobar foobar foobar" ].sort, result.get_field_array("artist").sort);

    result = search("/?query=title:something%20-artist:foobar")
    assert_equal([ "1 whatever" ].sort, result.get_field_array("artist").sort)


    # simple query, no filter
    result = search("/?query=title:same")
    ...
    result = search("/?query=title:same&filter=artist:foobar")
    assert_equal(5, result.hit.size)
