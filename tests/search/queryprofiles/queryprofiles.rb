...
class QueryProfiles < IndexedSearchTest
  ...
  def test_queryprofiles
    ...
    # This uses the "default" query profile, which sets default-index to "title", causing 20 hits
    assert_hitcount("query=best", 20)

    # This query profile however, sets default-index to "default", causing 60 hits
    assert_hitcount("query=best&queryProfile=root",60)

    # ...but the query profile setting can be overridden in the query
    assert_hitcount("query=best&default-index=title&queryProfile=root",20)

    # ...and using the full name works too
    assert_hitcount("query=best&model.defaultIndex=title&queryProfile=root",20)

    # ...and so	does setting the value which is used in the substitution
    assert_hitcount("query=best&indexname=title&queryProfile=root",20)

    # ...and so does alias 1
    assert_hitcount("query=best&Index-name=title&queryProfile=root",20)

    # ...and alias 2
    assert_hitcount("query=best&IDX=title&queryProfile=root",20)

    # ...but by using a query profile which makes the default-index unoverridable, the setting is ignored
    assert_hitcount("query=best&default-index=title&queryProfile=root/unoverridableIndex",60)

    # ...again same result with full name
    assert_hitcount("query=best&model.defaultIndex=title&queryProfile=root/unoverridableIndex",60)

    # This query profile does not exist, but should resolve to "root", because the type of root specified path matching
    assert_hitcount("query=best&queryProfile=root/nonexisting",60)
    ...
    assert_hitcount("query=best&queryProfile=rootStrict&default-index=title",20)
    ...
    assert_hitcount("query=best&queryProfile=mandatorySpecified",60)

    # Testing instance inheritance - this profile inherits all settings from root (punny name unintentional)
    assert_hitcount("query=best&queryProfile=rootChild",60)
    ...
    # The strange addendum at the end of each line is to trick the test framework into not adding 
    # query= to the front of the request string
    assert_hitcount("queryProfile=multi&ignore=query=",20); # query=title:best
    assert_hitcount("queryProfile=multi&myindex=default&myquery=love&ignore=query=",204); # query=default:love
    assert_hitcount("queryProfile=multi&myindex=default&ignore=query=",60); # query=default:best
    assert_hitcount("queryProfile=multi&myindex=default&myquery=notmatched&ignore=query=",60); # query=default:best
    ...
    assert_hitcount("queryProfile=multi&myindex=notmatched&myquery=notmatched&ignore=query=",20); # query=title:best
    assert_hitcount("queryProfile=multi&myquery=inheritslove&ignore=query=",45); # query=default:best filter=+me
