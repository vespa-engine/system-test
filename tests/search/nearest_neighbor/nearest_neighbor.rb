...
class NearestNeighborTest < IndexedSearchTest
  ...
  def test_nearest_neighbor_operator
    ...
    run_hnsw_tests("pos", "qpos_double")
  end

  def run_common_tests(doc_tensor, query_tensor, approx)
    assert_nearest_docs(-2, 10, doc_tensor, query_tensor, approx, nil, [[0,2],[1,3],[2,4],[3,5],[4,6],[5,7],[6,8],[7,9],[8,10],[9,11]])
    assert_nearest_docs(-2, 5,  doc_tensor, query_tensor, approx, nil, [[0,2],[1,3],[2,4],[3,5],[4,6]])
    assert_nearest_docs(-2, 3,  doc_tensor, query_tensor, approx, nil, [[0,2],[1,3],[2,4]])
    assert_nearest_docs(-2, 1,  doc_tensor, query_tensor, approx, nil, [[0,2]])
  end

  def run_brute_force_tests(doc_tensor, query_tensor)
    run_common_tests(doc_tensor, query_tensor, "false")
    ...
    assert_nearest_docs(11, 3,  doc_tensor, query_tensor, "false", nil, [[9,2],[8,3],[7,4],[6,5],[5,6],[4,7],[3,8],[2,9],[1,10],[0,11]])

    # With additional query filter
    assert_nearest_docs(-2, 3, doc_tensor, query_tensor, "false", 0, [[0,2],[2,4],[4,6]])
    assert_nearest_docs(-2, 3, doc_tensor, query_tensor, "false", 1, [[1,3],[3,5],[5,7]])
  end

  def run_hnsw_tests(doc_tensor, query_tensor)
    run_common_tests(doc_tensor, query_tensor, "true")
    run_common_tests(doc_tensor, query_tensor, nil)
    ...
    assert_nearest_docs(11, 3,  doc_tensor, query_tensor, nil, nil, [[9,2],[8,3],[7,4]])
    assert_nearest_docs(7,  1,  doc_tensor, query_tensor, nil, nil, [[7,0]])
    ...
    # fitered out:
    assert_nearest_docs(-2, 4, doc_tensor, query_tensor, nil, 0, [[0,2],[2,4]])
    assert_nearest_docs(-2, 4, doc_tensor, query_tensor, nil, 1, [[1,3],[3,5]])
    # Bruteforce is used when iterator is NOT strict:
    assert_nearest_docs(-2, 6, doc_tensor, query_tensor, nil, 0, [[0,2],[2,4],[4,6],[6,8],[8,10]])
    assert_nearest_docs(-2, 6, doc_tensor, query_tensor, nil, 1, [[1,3],[3,5],[5,7],[7,9],[9,11]])
  end
  ...
  def assert_nearest_docs(x_0, target_num_hits, doc_tensor, query_tensor, approx, filter, exp_results)
    query = get_query(x_0, X_1_POS, target_num_hits, doc_tensor, query_tensor, approx, filter)
  ...
  def get_query(x_0, x_1, target_num_hits, doc_tensor, query_tensor, approx = nil, filter = nil)
