...
class Explain < IndexedOnlySearchTest
  ...
  def verify_to_dispatch(result)
    ...
    assert_match(/basicsearch search to dispatch: query=\[\[documentmetastore\]:\*music\*\] timeout=[0-9]+ms offset=0 hits=1 groupingSessionCache=true sessionId=[0-9a-f\-]+.[0-9]+.[0-9].default restrict=\[music\]/, to_dispatch)
  end
