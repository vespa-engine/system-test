...
class Alias < IndexedSearchTest
  ...
  def comp(q, f, re)
    assert_result_matches(q, selfdir+f, re, true)
  end
  ...
  def test_alias
    ...
    comp("query=hate", "defaultalias.result", regexp)

    puts "Query: Search using explicit default index"
    comp("query=default:hate", "defaultalias.result", regexp)

    puts "Query: Search using alias 'testalias1' for index"
    comp("query=testalias1:hate", "defaultalias.result", regexp)
    ...
    comp("query=song:hate", "songalias.result", regexp)

    puts "Query: Search using 'default-index=song'"
    comp("query=hate&default-index=song", "songalias.result", regexp)
    ...
    comp("query=hate%20year:2000", "yearalias.result", regexp)

    puts "Query: Search using explicit alias 'testalias2' for index"
    comp("query=hate%20testalias2:2000", "yearalias.result", regexp)

    puts "Query: Search using 'default-index=year'"
    comp("query=default:hate+2000&default-index=year", "yearalias.result", regexp)

    puts "Query: Search using 'default-index=testalias2'"
    comp("query=testalias1:hate+2000&default-index=testalias2", "yearalias.result", regexp)

    puts "Query: Search using an attribute"
    comp("query=hate+weight:1700000",                   "weight.result", regexp)

    puts "Query: Search using alias for an attribute"
    comp("query=hate+testalias3:1700000",               "weight.result", regexp)

    puts "Query: Search using an attribute via default-index"
    comp("query=1700000&default-index=weight",     "weight.result", regexp)

    puts "Query: Search using alias for an attribute via default-index"
    comp("query=1700000&default-index=testalias3", "weight.result", regexp)
