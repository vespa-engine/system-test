...
class FieldSearch < IndexedSearchTest
  ...
  def test_medium_advanced
    ...
    assert_result_with_timeout(30, "query=rock%20year:%5B1999%3B2002%5D&hits=100", selfdir + "1.result.json", "surl", ["surl"])

    puts "Query: rock + range of years"
    compare("query=rock%20year:%5B1999%3B2002%5D&hits=100", "1.result.json", "surl")

    puts "Query: range of years - single year in range"
    compare("query=year:%5B1970%3B1973%5D%20-year:1972&hits=100", "2.result.json", "surl")

    puts "Query: rock - rock in title"
    compare("query=rock%20-title:rock&hits=100", "3.result.json", "surl")

    puts "Query: yellow in song - yellow in title"
    compare("query=song:yellow%20-title:yellow", "4.result.json", "surl")
    ...
    compare("query=year:%5B1%3B1929%5D", "6.result.json", "surl")

    puts "Query: searching for integer in text-fields"
    compare("query=title:101%20-song:101", "7.result.json", "surl")
  end
