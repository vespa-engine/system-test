...
class DistanceRanking < IndexedSearchTest
  ...
  def start_feed_and_check
    ...
    run_query("query=something&location=(#{loc},4199183343)", selfdir+"some.result")
    run_query("query=something&location=(#{loc},CalcLatLon)", selfdir+"some.result")
    run_query("query=something&pos.ll=N12.123123%3BE98.987987", selfdir+"some.result")

    # no ranking, so sort results by id
    qq = "query=poi&hits=99"
    run_query("#{qq}&sorting=id", selfdir+"allhits.result")

    loc = "location=[2,-118000000,33000000,-116000000,34000000]"
    run_query("#{qq}&#{loc}", selfdir+"dist_cutoff.result", "id")
    run_query("#{qq}&#{loc}", selfdir+"dist_cutoff.result", "id")
    ...
    run_query("#{qq}&location=(2,#{loc})", selfdir+"distranking.result")
    run_query("#{qq}&location=(2,#{loc},3579690824)", selfdir+"aspectranking.result")
    run_query("#{qq}&location=(2,#{loc},CalcLatLon)", selfdir+"aspectranking.result")

    loc = "pos.ll=N33.544000%3BW117.752900"
    run_query("#{qq}&#{loc}&ranking=default", selfdir+"dist2a.result")
    run_query("#{qq}&#{loc}&ranking=nearby",  selfdir+"dist2b.result")
    run_query("#{qq}&#{loc}&ranking=combine", selfdir+"dist2c.result")
  end
